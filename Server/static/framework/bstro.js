 ...
 * qiao.totop
 * 杩斿洖椤堕儴鐨勬柟娉�
 * 鍙互鍙傝€冿細plugins/_01_qtotop/qtotop.html
 */
 ...
 * qiao.qcode
 * 鐢熸垚浜岀淮鐮�
 * 娉細闇€瑕佸紩鍏rcode锛�<script src="http://cdn.staticfile.org/jquery.qrcode/1.0/jquery.qrcode.min.js"></script>
 * text锛氬緟鐢熸垚鏂囧瓧
 * type锛氫腑鏂囪繕鏄嫳鏂囷紝cn涓轰腑鏂�
 * render锛氬睍绀烘柟寮忥紝table涓鸿〃鏍兼柟寮�
 * width锛氬搴�
 * height锛氶珮搴�
 * 鍙互鍙傝€冿細plugins/_03_qcode/qcode.html
 */
 ...
 * end锛氬埌杈惧簳閮ㄥ悗鐨勫洖璋冨嚱鏁�
 * $d锛氬鍣紝榛樿鏄�$(window)
 * $c锛氬唴瀹癸紝榛樿鏄�$(document)
 * 鍙互鍙傝€冿細plugins/_04_qend/qend.html
 */
...
/**
 * qiao.cookie
 * 瀵筳query.cookie.js绋嶄綔灏佽
 * 娉細闇€瑕佸紩鍏query.cookie.js锛�<script src="http://cdn.bootcss.com/jquery-cookie/1.4.1/jquery.cookie.min.js"></script>
 * qiao.cookie(key)锛氳繑鍥瀔ey瀵瑰簲鐨剉alue
 * qiao.cookie(key, null)锛� 鍒犻櫎key瀵瑰簲鐨刢ookie
 * qiao.cookie(key, value)锛氳缃甼ey-value鐨刢ookie
 * 鍙互鍙傝€冿細plugins/_05_qcookie/qcookie.html
 */
 ...
 * qiao.search
 * 鑾峰彇url鍚庡弬鏁颁腑鐨剉alue
 * qiao.search(key)锛氳繑鍥炲弬鏁颁腑key瀵瑰簲鐨剉alue
 * 鍙互鍙傝€冿細plugins/_06_qsearch/qsearch.html
 */
...
qiao.bs.tree.sub = function(tree, opt){
	...
	if(tree){
		...
		if(opt.edit)
			res += 
				'&nbsp;&nbsp;<span class="label label-primary bstreeadd">娣诲姞瀛愯彍鍗�</span>' + 
				'&nbsp;&nbsp;<span class="label label-primary bstreeedit">淇敼</span>' + 
				'&nbsp;&nbsp;<span class="label label-danger  bstreedel">鍒犻櫎</span>';
			res += '</a>';
...
qiao.bs.bstroopts = {
	prevButtonText : '上一步',
	finishButton : '<button class="btn btn-lg btn-success bootstro-finish-btn"><i class="icon-ok"></i>结束</button>',
...
qiao.crud.bindcrud = function(){
	...
	qiao.on('.addBtn', 'click', function(){qiao.crud.savep('娣诲姞')});
	qiao.on('.editbtn','click', function(){qiao.crud.savep('淇敼',$(this).parents('tr').qdata().id)});
	qiao.on('.queBtn', 'click', function(){qiao.crud.search('鏌ヨ')});
...
qiao.crud.del = function(id){
	...
	if(!ids.length){
		qiao.bs.alert('璇烽€夋嫨瑕佸垹闄ょ殑璁板綍锛�');
	}else{
		qiao.bs.confirm('纭瑕佸垹闄ゆ墍閫夎褰曞悧锛堣嫢鏈夊瓙璁板綍涔熶細鍚屾椂鍒犻櫎锛夛紵',function(){
			qiao.ajax({
